...
class PhpDocs extends Object implements Command {
	...
	function execute() {
		...
		if (!empty($_GET['generate_docs'])) { // Moet er documentation gegenereerd worden?
			...
			$success = $this->generate_docs($target_path); 
			if ($success) { // Is het genereren misukt?
				redirect($base_url); // Toon de documentatie
			} else {
				return false; // Toon de log
			}
		}
	...
	function generate_docs($target_path) {
		...
		if (strpos($phpdocs_output, '<h1>Operation Completed!!</h1>')) {
			return true;
		} else {
			return new HTML($phpdocs_output);
		}
	}
}
?>
